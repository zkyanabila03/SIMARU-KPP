    ...
    with tab1:
        ...
        try:
            ...
            if not room_df.empty:
                ...
                display_df = room_df[['date', 'item_name', 'start_time', 'end_time', 'purpose', 'status']].copy()
                display_df.columns = ['Tanggal', 'Ruangan', 'Jam Mulai', 'Jam Selesai', 'Keperluan', 'Status']
    ...
    with tab2:
        ...
        try:
            ...
            if not asset_df.empty:
                # Format kolom untuk tampilan
                display_df = asset_df[['borrow_date', 'return_date', 'item_name', 'purpose', 'status']].copy()
                display_df.columns = ['Tanggal Pinjam', 'Tanggal Kembali', 'Aset', 'Keperluan', 'Status']
                display_df['Status'] = display_df['Status'].str.capitalize()
                st.dataframe(display_df, use_container_width=True)
            else:
    ...
    with tab3:
        ...
        try:
            ...
            if not vehicle_df.empty:
                # Format kolom untuk tampilan
                display_df = vehicle_df[['start_date', 'end_date', 'item_name', 'destination', 'purpose', 'status']].copy()
                display_df.columns = ['Tanggal Mulai', 'Tanggal Kembali', 'Kendaraan', 'Tujuan', 'Keperluan', 'Status']
                display_df['Status'] = display_df['Status'].str.capitalize()
                st.dataframe(display_df, use_container_width=True)
            else:
...
# ============================================================================
# HALAMAN ADMIN - HAPUS TAB PERSETUJUAN (karena semua langsung disetujui)
# ============================================================================
def show_admin():
    show_navbar()
...
def show_jadwal():
    ...
    with tab1:
        ...
        try:
            ...
            if not room_df.empty:
                # Filter untuk tanggal yang dipilih - PERBAIKI: pakai kolom 'date'
                filtered_df = room_df[room_df['date'] == str(selected_date)]
                
                if not filtered_df.empty:
                    display_df = filtered_df[['item_name', 'start_time', 'end_time', 'user_name', 'purpose', 'status']].copy()
    ...
    with tab2:
        ...
        try:
            ...
            if not asset_df.empty:
                # PERBAIKI: Sekarang kolomnya disebut 'date' bukan 'borrow_date'
                # Cek kolom yang tersedia
                print(f"DEBUG Asset columns: {asset_df.columns.tolist()}")
                
                # Filter untuk tanggal yang dipilih - PERBAIKI LOGIKA
                # Aset dipinjam jika selected_date antara borrow_date dan return_date
                # Kita perlu kolom 'date' (borrow_date) dan 'return_date'
                if 'date' in asset_df.columns and 'return_date' in asset_df.columns:
                    ...
                    if not filtered_df.empty:
                        ...
                        display_df = filtered_df[['item_name', 'user_name', 'purpose', 'status']].copy()
                        
                        # Tambahkan tanggal jika kolom tersedia
                        if 'date' in filtered_df.columns:
                            display_df['Tanggal Pinjam'] = filtered_df['date']
                        if 'return_date' in filtered_df.columns:
                            display_df['Tanggal Kembali'] = filtered_df['return_date']
                        
                        # Atur urutan kolom
                        if 'Tanggal Pinjam' in display_df.columns and 'Tanggal Kembali' in display_df.columns:
                            display_df = display_df[['item_name', 'Tanggal Pinjam', 'Tanggal Kembali', 'user_name', 'purpose', 'status']]
                            display_df.columns = ['Aset', 'Tanggal Pinjam', 'Tanggal Kembali', 'Pemesan', 'Keperluan', 'Status']
                        else:
                            display_df.columns = ['Aset', 'Pemesan', 'Keperluan', 'Status']
                    ...
                    st.warning("Kolom tanggal tidak ditemukan dalam data aset")
    ...
    with tab3:
        ...
        try:
            ...
            if not vehicle_df.empty:
                # PERBAIKI: Sekarang kolomnya disebut 'date' bukan 'start_date'
                # Cek kolom yang tersedia
                print(f"DEBUG Vehicle columns: {vehicle_df.columns.tolist()}")
                ...
                if 'date' in vehicle_df.columns:
                    ...
                    st.warning("Kolom tanggal tidak ditemukan dalam data kendaraan")
