...
class Database:
    ...
    def insert_initial_data(self):
        ...
        if cursor.fetchone()[0] == 0:
            assets = [
                ...
                ('Proyektor Epson 1', 'Proyektor'),
                ('Proyektor Epson 2', 'Proyektor'),
            ]
    ...
    def add_room_booking(self, user_id, room_id, date, start_time, end_time, purpose):
        """Tambah booking ruangan - LANGSUNG DISETUJUI"""
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO room_bookings (user_id, room_id, date, start_time, end_time, purpose, status)
                VALUES (?, ?, ?, ?, ?, ?, 'disetujui')
            """, (user_id, room_id, date, start_time, end_time, purpose))
            conn.commit()
            conn.close()
            return True
        except Exception as e:
    ...
    def add_asset_booking(self, user_id, asset_id, borrow_date, return_date, purpose):
        """Tambah booking aset - LANGSUNG DISETUJUI"""
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO asset_bookings (user_id, asset_id, borrow_date, return_date, purpose, status)
                VALUES (?, ?, ?, ?, ?, 'disetujui')
            """, (user_id, asset_id, borrow_date, return_date, purpose))
            conn.commit()
            conn.close()
            return True
        except Exception as e:
    ...
    def add_vehicle_booking(self, user_id, vehicle_id, start_date, end_date, start_time, end_time, destination, purpose):
        """Tambah booking kendaraan - LANGSUNG DISETUJUI"""
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO vehicle_bookings (user_id, vehicle_id, start_date, end_date, start_time, end_time, destination, purpose, status)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'disetujui')
            """, (user_id, vehicle_id, start_date, end_date, start_time, end_time, destination, purpose))
            conn.commit()
            conn.close()
            return True
        except Exception as e:
    ...
    def get_all_bookings(self):
        """Ambil semua booking untuk admin - PERBAIKAN NAMA KOLOM"""
        conn = self.get_connection()
        
        try:
            # Ruangan - KOLOM: date, start_time, end_time
            room_df = pd.read_sql_query("""
                SELECT 
                    rb.id,
                    rb.date,
            ...
            # Aset - PERBAIKI: ganti borrow_date menjadi date
            asset_df = pd.read_sql_query("""
                SELECT 
                    ab.id,
                    ab.borrow_date as date,  -- RENAME menjadi 'date'
                    ab.return_date,
                    a.name as item_name,
                    u.name as user_name,
                    ab.purpose,
            ...
            # Kendaraan - PERBAIKI: ganti start_date menjadi date
            vehicle_df = pd.read_sql_query("""
                SELECT 
                    vb.id,
                    vb.start_date as date,  -- RENAME menjadi 'date'
                    vb.end_date,
                    vb.start_time,
                    vb.end_time,
                    v.name as item_name,
    ...
    def export_bookings_to_csv(self, csv_path="booking.csv"):
        """Export semua data booking ke file CSV"""
        try:
            # Ambil semua data booking
            room_df, asset_df, vehicle_df = self.get_all_bookings()
            
            # Buat DataFrame untuk masing-masing tipe
            # Ruangan
            room_export = room_df[['date', 'start_time', 'end_time', 'item_name', 'user_name', 'purpose', 'status', 'created_at']].copy()
            room_export.columns = ['Tanggal', 'Jam Mulai', 'Jam Selesai', 'Ruangan', 'Pemesan', 'Keperluan', 'Status', 'Waktu Booking']
            room_export['Jenis'] = 'Ruangan'
            
            # Aset
            asset_export = asset_df[['borrow_date', 'return_date', 'item_name', 'user_name', 'purpose', 'status', 'created_at']].copy()
            asset_export.columns = ['Tanggal Pinjam', 'Tanggal Kembali', 'Aset', 'Pemesan', 'Keperluan', 'Status', 'Waktu Booking']
            asset_export['Jenis'] = 'Aset'
            
            # Kendaraan
            vehicle_export = vehicle_df[['start_date', 'end_date', 'item_name', 'user_name', 'destination', 'purpose', 'status', 'created_at']].copy()
            vehicle_export.columns = ['Tanggal Mulai', 'Tanggal Kembali', 'Kendaraan', 'Pemesan', 'Tujuan', 'Keperluan', 'Status', 'Waktu Booking']
            vehicle_export['Jenis'] = 'Kendaraan'
            ...
            all_bookings = all_bookings.sort_values('Waktu Booking', ascending=False)
            
            # Simpan ke CSV
            all_bookings.to_csv(csv_path, index=False, encoding='utf-8')
            
            print(f"Data berhasil diexport ke {csv_path}")
            return True
    ...
    def export_daily_bookings(self, date_str, csv_path="booking_harian.csv"):
        ...
        try:
            ...
            daily_assets = asset_df[
                (asset_df['borrow_date'] <= date_str) & 
                (asset_df['return_date'] >= date_str)
            ]
            daily_vehicles = vehicle_df[
                (vehicle_df['start_date'] <= date_str) & 
            ...
            for _, row in daily_rooms.iterrows():
                all_daily.append({
                    'Jenis': 'Ruangan',
                    'Item': row['item_name'],
                    'Pemesan': row['user_name'],
                    'Waktu': f"{row['start_time']} - {row['end_time']}",
            ...
            for _, row in daily_assets.iterrows():
                all_daily.append({
                    ...
                    'Item': row['item_name'],
                    'Pemesan': row['user_name'],
                    'Waktu': f"{row['borrow_date']} s/d {row['return_date']}",
                    'Keperluan': row['purpose'],
                    'Status': row['status']
                })
            ...
            for _, row in daily_vehicles.iterrows():
                all_daily.append({
                    ...
                    'Item': row['item_name'],
                    'Pemesan': row['user_name'],
                    'Waktu': f"{row['start_date']} s/d {row['end_date']}",
                    'Keperluan': row['purpose'],
                    'Status': row['status']
                })
            ...
            if all_daily:
                ...
                print(f"Data harian berhasil diexport ke {csv_path}")
                return True
            else:
                print("Tidak ada data untuk tanggal tersebut")
                return False
